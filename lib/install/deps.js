...
function prefetchDeps (tree, deps, log, next) {
  ...
  var seen = {}
  resolveBranchDeps(tree.package, deps, andFinishTracker(log, next))

  function resolveBranchDeps (pkg, deps, next) {
    var allDependencies = Object.keys(deps).map((dep) => npa.resolve(dep, deps[dep]))
      .filter((dep) => dep.registry && !seen[dep.toString()] && !findRequirement(tree, dep.name, dep))
    if (skipOptional) {
      var optDeps = pkg.optionalDependencies || {}
      allDependencies = allDependencies.filter((dep) => !optDeps[dep.name])
    }
    asyncMap(allDependencies, function (dep, done) {
      seen[dep.toString()] = true
      fetchPackageMetadata(dep, '', {tracker: log.newItem('fetchMetadata')}, function (err, pkg) {
        if (err || !pkg || !pkg.dependencies) return done()
        return resolveBranchDeps(pkg, pkg.dependencies, done)
      })
    }, next)
  }
}
